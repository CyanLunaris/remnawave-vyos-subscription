...
class TestRouting:
    ...
    def test_sniff_timeout_plain_number_is_normalized_to_ms(self):
        cfg = generate_config(make_vless_reality(), ConfigSettings(route_sniff_timeout="100"))
        assert cfg["route"]["rules"][0]["timeout"] == "100ms"

    def test_sniff_timeout_invalid_value_falls_back_to_default(self):
        cfg = generate_config(make_vless_reality(), ConfigSettings(route_sniff_timeout="fast"))
        assert cfg["route"]["rules"][0]["timeout"] == "100ms"
